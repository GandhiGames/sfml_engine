...
void Map::LoadMap(const std::string& l_path){
    ...
    while(std::getline(mapFile,line)){
        ...
        if(type == "TILE"){
            ...
            if (!m_context.GetTextureManager()->RequireResource(m_backgroundTexture)){
                m_backgroundTexture = "";
                continue;
            }
            sf::Texture* texture = m_context.GetTextureManager()->GetResource(m_backgroundTexture);
