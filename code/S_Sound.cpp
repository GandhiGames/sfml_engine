    ...
    bool l_useId, bool l_relative, int l_checkFrame)F
{
    ...
    if (l_useId){ c_snd->SetSoundID(m_soundManager->PlayOneShot(c_snd->GetSound(l_sound), pos)); }
    else { m_soundManager->PlayOneShot(c_snd->GetSound(l_sound), pos, false, l_relative); }
}
