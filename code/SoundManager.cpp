...
{
    m_elapsed += l_dT;
    if (m_elapsed < 0.33f){ return; } // Run once every third of a second.
    m_elapsed = 0;
...
bool SoundManager::PlayMusic(const std::string& l_musicId, float l_volume, bool l_loop)
{
    ...
    if (!music->openFromFile(resourcePath() + path)){
        delete music;
        --m_numSounds;
...
//TODO(robert): Refractorif statement into function collection: std::unordered_map<std::string, <std::function<>>> 
bool SoundManager::LoadProperties(const std::string& l_name)
{
    std::ifstream file;
    file.open(resourcePath() + "media/Sounds/" + l_name + ".sound");
    if (!file.is_open()){
    ...
    while (std::getline(file, line)){
        ...
        if (type == "Audio"){
            ...
            keystream >> props.m_minDist;
        } else if (type == "Attenuation"){
            keystream >> props.m_attenuation;
        } else {
            std::cerr << "SoundManager::LoadProperties - Invalid Property" << std::endl;
        }
    }
    file.close();
...
void SoundManager::SetUpSound(sf::Sound* l_snd, const SoundProps* l_props, bool l_loop, bool l_relative)
{
    ...
    l_snd->setMinDistance(l_props->m_minDist);
