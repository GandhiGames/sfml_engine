...
//TODO(robert): Split into different classes for music and sound effects.
//TODO(robert): When user changes sound output to headphones the sound should switch channel.
class SoundManager
{
public:
    ...
    SoundID PlayOneShot(const std::string& l_sound, 
        const sf::Vector3f& l_position, 
        bool l_loop = false, 
        bool l_relative = false);
    bool PlayOneShot(const SoundID& l_id);
